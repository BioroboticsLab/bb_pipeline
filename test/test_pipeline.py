...
from bb_binary import DataSource, FrameContainer, Repository
...
def test_padding(pipeline_config):
    ...
    for padded, original in zip(
        outputs[PaddedLocalizerPositions], outputs[LocalizerPositions]
    ):
...
def test_decoder(pipeline_config):
    pipeline = Pipeline([Filename], [LocalizerPositions, IDs], **pipeline_config)
...
@pytest.mark.slow
def test_generator_processor_threads(
    tmpdir, bees_video, filelists_path, pipeline_config
):
    repo = Repository(str(tmpdir))
    pipelines = [
...
def test_crown_visualiser_on_a_bee(bee_in_the_center_image, outdir):
    bee_img = imread(bee_in_the_center_image) / 255.0
...
def test_crown_visualiser_on_a_image(pipeline_results, bees_image, outdir):
    ...
    res = pipeline_results
    img = res[Image]
    (overlay,) = vis(res[Image], res[LocalizerPositions], res[Orientations], res[IDs])
    overlay = zoom(overlay, (0.5, 0.5, 1), order=1)
    img = zoom(img, 0.5, order=3) / 255.0
