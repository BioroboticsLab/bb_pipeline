...
class VideoReader:
    def __init__(self,
        ...
        vidread_command = [
            ffmpeg_bin, '-i', video_path, '-f', 'image2pipe', '-pix_fmt',
            'gray', '-vsync', '0', '-vcodec', 'rawvideo', '-'
        ]
        ...
        resolution_command = [
            ffprobe_bin, '-v', 'error', '-of', 'flat=s=_', '-select_streams',
            'v:0', '-show_entries', 'stream=height,width', video_path
        ]

        pipe = sp.Popen(resolution_command, stdout=sp.PIPE, stderr=sp.PIPE)
        infos = pipe.stdout.readlines()
        self.w, self.h = [
            int(s.decode('utf-8').strip().split('=')[1]) for s in infos
        ]
...
def get_seperate_timestamps(path_video, ts_format, path_filelists):
    ...
    fnames = image_fnames[image_fnames.index(first_fname):image_fnames.
                          index(second_fname) + 1]
    return [
