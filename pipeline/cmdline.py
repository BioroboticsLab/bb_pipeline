...
def get_shared_positional_arguments():
    arguments = list()
    arguments.append(
        ("timestamp_format", {"help": "format of timestamps (e.g. 2015)", "type": str})
    )
    arguments.append(
        (
            "repo_output_path",
            {"help": "root path for bb_binary output repository", "type": str},
        )
    )
    return arguments
...
def get_shared_optional_arguments():
    ...
    arguments.append(
        (
            "--progressbar",
            {
                "help": "show progressbar while processing",
                "type": bool,
                "default": True,
            },
        )
